<template>
  <view class="content page-order">
    ...
    <view class="section-order-price-bar bg-white">
      ...
      <view class="price-item text-size-basic">
        <text>会员折扣</text>
        <text>-¥{{((order.total_amount-order.freight)* ((10-userInfo.mbr_dis_count) / 10)).toFixed(2)}}</text>
      </view>
      <view class="price-item text-size-basic">
        <text>运费</text>
    ...
    <view class="section-btns" @click="openPopup">
      <operationButton :price="(order.freight+(order.total_amount-order.freight)*userInfo.mbr_dis_count / 10).toFixed(2)" buttonText="去支付"></operationButton>
    </view>
    <!-- popup s -->
