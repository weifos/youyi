<template>
  <view class="component-location">
    <view :class="['city-cur',show == true ? 'show' : '']" @click="showLayout">
      所选城市：
      <text>{{selected}}</text>
    </view>
    <view class="city-list" v-if="show">
      <view @click="selectItem(item)" :class="['list-item',selected == item ? 'cur' : '']" v-for="item in cityList" :key="item">
        <view class="yoyi-tag">{{item}}</view>
      </view>
    </view>
  </view>
</template>
...
export default {
  data() {
    return {
      selected: "深圳市",
      cityList: ["深圳市", "广州市", "东莞市", "中山市", "惠州市", "佛山市"],
      show: false
    }
  },
  props: {
